...
public interface LastMessageRepository extends JpaRepository<lastMessage, Integer> {
    ...
    @Query("SELECT new com.example.chatting_app_backend.data_packets.Responses.ActiveChatsResponse(lm.user2, lm.lastMessage) FROM lastMessage lm WHERE lm.user1 = ?1")
